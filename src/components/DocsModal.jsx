...
export default function DocsModal({ activeModal, onClose }) {
    const [searchTerm, setSearchTerm] = useState('');
    const [activeDoc, setActiveDoc] = useState(null);

    if (activeModal !== 'docs-modal') return null;
            ...
            <div className="modal" style={{ maxWidth: '640px' }}>
                ...
                {!activeDoc ? (
                    <div id="docs-index">
                        ...
                        <div className="modal-sub" style={{ marginBottom: '20px' }}>Everything you need to integrate Orclave Layer.</div>
                        ...
                        <div style={{ position: 'relative', marginBottom: '20px' }}>
                            ...
                            <input
                                style={{ width: '100%', padding: '11px 14px 11px 40px', background: 'var(--bg3)', border: '1px solid var(--border2)', borderRadius: '10px', color: 'var(--text)', fontSize: '13px', fontFamily: "'DM Sans', sans-serif", outline: 'none' }}
                        ...
                        <div id="docs-list-wrap">
                            {gettingStartedDocs.length > 0 && (
                                <>
                                    <div style={{ fontSize: '10px', fontWeight: 700, color: 'var(--text-dim)', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: '8px' }}>Getting Started</div>
                                    {gettingStartedDocs.map(doc => (
                                        <a key={doc.key} className="docs-item" href="#" onClick={(e) => { e.preventDefault(); setActiveDoc(doc.key); }}>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                                <div style={{ width: '30px', height: '30px', borderRadius: '8px', background: 'rgba(201,168,76,0.08)', border: '1px solid rgba(201,168,76,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                                                    {doc.icon}
                                                </div>
                                                <div>
                                                    <div style={{ fontSize: '13px', color: 'var(--text)', fontWeight: 500 }}>{doc.title}</div>
                                                    <div style={{ fontSize: '11px', color: 'var(--text-muted)', marginTop: '1px' }}>{doc.desc}</div>
                                                </div>
                                            </div>
                                            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#4A4845" strokeWidth="2"><path d="M5 12h14M12 5l7 7-7 7" /></svg>
                                        </a>
                                    ))}
                                </>
                            )}
                            ...
                            {guideDocs.length > 0 && (
                                <>
                                    <div style={{ fontSize: '10px', fontWeight: 700, color: 'var(--text-dim)', letterSpacing: '0.1em', textTransform: 'uppercase', margin: '16px 0 8px' }}>Guides</div>
                                    {guideDocs.map(doc => (
                                        <a key={doc.key} className="docs-item" href="#" onClick={(e) => { e.preventDefault(); setActiveDoc(doc.key); }}>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                                <div style={{ width: '30px', height: '30px', borderRadius: '8px', background: 'rgba(201,168,76,0.08)', border: '1px solid rgba(201,168,76,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    ...
                    <div id="doc-page-view">
                        <button
                            ...
                            style={{ display: 'flex', alignItems: 'center', gap: '8px', background: 'none', border: 'none', color: 'var(--gold)', fontSize: '13px', fontFamily: "'DM Sans', sans-serif", cursor: 'pointer', marginBottom: '22px', padding: 0 }}
                        >
                        ...
                        <div id="doc-page-body">
                            <div style={{ display: 'inline-block', padding: '3px 10px', background: 'rgba(201,168,76,0.1)', border: '1px solid rgba(201,168,76,0.2)', borderRadius: '6px', fontSize: '11px', color: 'var(--gold)', fontWeight: 600, marginBottom: '16px' }}>{docContents[activeDoc].readingTime}</div>
                            <div style={{ fontFamily: "'Syne', sans-serif", fontSize: '20px', fontWeight: 700, color: 'var(--text)', letterSpacing: '-0.5px', marginBottom: '6px' }}>{docContents[activeDoc].title}</div>
                            <div className="doc-p">{docContents[activeDoc].desc}</div>
